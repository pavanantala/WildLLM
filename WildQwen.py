#!/usr/bin/env python3
"""
MCC-Focused Hyperparameter Tuning for Qwen2.5-7B with LoRA
===========================================================

This script performs comprehensive hyperparameter tuning for tweet classification
using Qwen2.5-7B with LoRA (Low-Rank Adaptation). The primary evaluation metric
...
def plot_comparative_results(all_results, output_dir):
    ...
    plt.title('MCC Score Comparison Across Different Hyperparameters (Qwen2.5-7B - PRIMARY METRIC)')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(comparison_dir, 'qwen_mcc_comparison_primary.pdf'))
    plt.savefig(os.path.join(comparison_dir, 'qwen_mcc_comparison_primary.png'), dpi=300)
    plt.close()
    ...
    plt.xlabel('Hyperparameters')
    plt.ylabel('Score')
    plt.title('Comprehensive Performance Metrics Comparison (Qwen2.5-7B - MCC as Primary)')
    plt.xticks(x, configs, rotation=45, ha='right')
    plt.ylim(-1.0, 1.0)
    plt.legend()
    plt.axhline(y=0, color='gray', linestyle='--', alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(comparison_dir, 'qwen_comprehensive_metrics_mcc_focused.pdf'))
    plt.close()
    
    # Best model performance
    best_model = max(all_results, key=lambda x: x['best_val_mcc'])
    ...
    plt.ylim(-1.0, 1.0)
    plt.title(f'Best Qwen2.5-7B Model Performance (Selected by MCC): {best_config}')
    plt.ylabel('Score')
    plt.axhline(y=0, color='gray', linestyle='--', alpha=0.3)
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join(comparison_dir, 'qwen_best_model_mcc_focused.pdf'))
    plt.close()
...
def run_experiment(config, train_df, val_df, test_df):
    ...
    logger.info("Starting Qwen2.5-7B training with epoch-wise test evaluation...")
    ...
    for epoch in range(config['epoch']):
        ...
        if val_metrics['mcc'] > best_val_mcc:
            ...
            logger.info(f"New best Qwen2.5-7B model saved! (Val MCC: {best_val_mcc:.4f})")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss Curves (Qwen2.5-7B: lr={config["lr"]}, bs={config["batch_size"]})')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MCC Score')
    plt.title(f'MCC Progression (Qwen2.5-7B: lr={config["lr"]}, bs={config["batch_size"]})')
    plt.legend()
    ...
    logger.info("Evaluating best Qwen2.5-7B model on test set...")
...
def main():
    parser = argparse.ArgumentParser(description='Run MCC-focused hyperparameter tuning for Qwen2.5-7B LoRA')
    parser.add_argument('--data_path', type=str, default='data/',
                        help='Path to data directory containing train.csv, val.csv, test.csv')
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Output directory (default: LLM/qwen_llm_tuning_TIMESTAMP)')
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("LLM/qwen_mcc_focused_llm_tuning.log"),
    ...
    if args.output_dir is None:
        ...
        output_dir = f"LLM/qwen_llm_tuning_{timestamp}"
    else:
    ...
    main_logger.info(f"Running {len(param_combinations)} Qwen2.5-7B experiments")
    ...
    for lr, batch_size in param_combinations:
        ...
        main_logger.info(f"Starting Qwen2.5-7B experiment with lr={lr}, batch_size={batch_size}")
    ...
    best_model_dest = os.path.join(output_dir, "best_qwen_model.pkl")
    shutil.copy2(best_model_source, best_model_dest)
    
    # Save all results
    with open(os.path.join(output_dir, 'qwen_all_results.json'), 'w') as f:
    ...
    print("\n" + "="*70)
    print("MCC-Focused Qwen2.5-7B Hyperparameter Tuning Completed Successfully!")
